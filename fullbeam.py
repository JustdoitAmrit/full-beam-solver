...
# Initialize or retrieve supports, loads, and messages in session state
...
if 'messages' not in st.session_state:
    st.session_state.messages = []  # Store messages persistently
...
class Beam:
    def __init__(self, young, inertia, length, segments):
        ...
        self.udl = np.zeros((self.segments + 1, 2))
    ...
    def analysis(self):
        ...
        p = self.point_load.flatten() + self.udl.flatten()  # Include UDL in loads
        pf = p[free_dof]
    ...
    def plot(self):
        ...
        self.analysis()  # Run analysis for plotting
...
if add_support:
    ...
    st.session_state.messages.append(f"{support_type} support added at position {support_position}")
...
if add_load:
    ...
    st.session_state.messages.append(f"Point load of {load_magnitude} N added at position {load_position}")

# Display added point loads
...
# Adding Uniformly Distributed Load (UDL)
...
if st.sidebar.button("Add UDL"):
    ...
    st.session_state.messages.append(f"UDL of {udl_magnitude} N/m from {udl_start_position} to {udl_end_position}")
...
if calculate_button:
    ...
    for udl in st.session_state.udls:
        ...
        if start < end:
            beam.udl[start:end+1, 0] += magnitude  # Apply UDL in the specified range
